...
class VideoModelEncoder:
    ...
    def decode_hyper(self, z_strings):
        ...
        self.z_hat = self.model.hyper_bottleneck.decompress(z_strings, (8, 10))

        # Decode hyper-prior from latent hyper-prior
        self.hyper_params = self.model.hyper_synthesis(self.z_hat)
    ...
    def decode_image(self, y_strings):
        ...
        self.sigma_hat, self.means_hat = self.model.entropy_parameters(self.hyper_params).chunk(2, 1)

        # Build the scale indexes for quantization
        self.indexes = self.model.image_bottleneck.build_indexes(self.sigma_hat)

        # Dequantize the latent image with the scale indexes and hyper-prior means
        self.y = self.model.image_bottleneck.decompress(y_strings, self.indexes, means=self.means_hat)

        # Decode image from dequantized latent image
        self.x_hat = self.model.image_synthesis(self.y)
