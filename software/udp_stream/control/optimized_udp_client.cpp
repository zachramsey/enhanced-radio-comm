...
constexpr int DEFAULT_BUFFER_SIZE = 5;  // Reduced buffer size
constexpr int DEFAULT_DISPLAY_WIDTH = 320;  // Smaller display size
constexpr int DEFAULT_DISPLAY_HEIGHT = 240;
constexpr int DEFAULT_FRAME_SKIP = 1;  // Display every Nth frame
...
// Frame class to store complete frames
class Frame {
public:
    cv::Mat data;
    uint32_t frameNumber;
    
    Frame(const cv::Mat& mat, uint32_t num) : data(mat), frameNumber(num) {}
};
...
public:
    ...
    bool waitAndPop(Frame& frame, int timeoutMs) {
        std::unique_lock<std::mutex> lock(mutex);
        
        if (frames.empty()) {
            // Wait with timeout
            auto status = condition.wait_for(lock, std::chrono::milliseconds(timeoutMs),
...
private:
    ...
    size_t maxSize = DEFAULT_BUFFER_SIZE;
    bool running = true;
};
...
void printUsage(const char* programName) {
    std::cout << "Usage: " << programName << " [options]\n"
              ...
              << "  --display-width W Display width (default: " << DEFAULT_DISPLAY_WIDTH << ")\n"
              << "  --display-height H Display height (default: " << DEFAULT_DISPLAY_HEIGHT << ")\n"
              << "  --frame-skip N    Display every Nth frame (default: " << DEFAULT_FRAME_SKIP << ")\n"
              << "  --no-display      Don't display frames (for headless operation)\n"
...
int main(int argc, char* argv[]) {
    ...
    int frameSkip = DEFAULT_FRAME_SKIP;
    ...
    const struct option long_options[] = {
        ...
        {"frame-skip", required_argument, nullptr, 'f'},
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "p:s:b:w:h:f:n?", long_options, nullptr)) != -1) {
        switch (opt) {
            case 'p': port = std::stoi(optarg); break;
            case 's': serverIP = optarg; break;
            case 'b': bufferSize = std::stoi(optarg); break;
            case 'w': displayWidth = std::stoi(optarg); break;
            case 'h': displayHeight = std::stoi(optarg); break;
            case 'f': frameSkip = std::stoi(optarg); break;
            case 'n': displayFrames = false; break;
            case '?':
                printUsage(argv[0]);
                return 0;
            default:
    ...
    std::cout << "Display settings: " << displayWidth << "x" << displayHeight 
              << ", frame skip: " << frameSkip << std::endl;
    ...
    // Create frame buffer and queue
    FrameBuffer frameBuffer(bufferSize);
    FrameQueue displayQueue;
    displayQueue.setMaxSize(bufferSize);
    
    // Create window for display if needed
    if (displayFrames) {
        cv::namedWindow("UDP Video Stream", cv::WINDOW_NORMAL);
        cv::resizeWindow("UDP Video Stream", displayWidth, displayHeight);
    }
    ...
    std::atomic<uint32_t> lastProcessedFrame(0);
    ...
    std::thread processingThread([&]() {
        ...
        while (running) {
            ...
            if (frameBuffer.getNextFrame(frame)) {
                ...
                // Only process every Nth frame based on frame skip
                if (displayFrameCount % frameSkip == 0) {
                    // Resize frame for display if needed
                    cv::Mat resizedFrame;
                    if (displayFrames && (frame.cols != displayWidth || frame.rows != displayHeight)) {
                        cv::resize(frame, resizedFrame, cv::Size(displayWidth, displayHeight), 0, 0, cv::INTER_NEAREST);
                    } else {
                        resizedFrame = frame;
                    }
                    
                    // Add to display queue
                    displayQueue.push(Frame(resizedFrame, frameNumber));
                }
                
                lastProcessedFrame = frameNumber;
            } else {
                // Short sleep if no frames are available
    ...
    // Main display loop
    try {
        Frame displayFrame(cv::Mat(), 0);
        
        while (running) {
            // Get next frame for display
            if (displayFrames && displayQueue.waitAndPop(displayFrame, 100)) {
                // Update FPS counter
                frameCount++;
                ...
                std::string statsText = cv::format("FPS: %.1f | Data: %.2f Mbps | Frame: %u", 
                                                  fps, dataRateMbps, displayFrame.frameNumber);
                cv::putText(displayFrame.data, statsText, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 0.5, 
                           cv::Scalar(0, 255, 0), 1);
                
                // Display frame
                cv::imshow("UDP Video Stream", displayFrame.data);
                ...
                // If not displaying frames, just update stats periodically
                auto now = std::chrono::steady_clock::now();
                auto dataRateElapsed = std::chrono::duration_cast<std::chrono::seconds>(now - dataRateStartTime).count();
                if (dataRateElapsed >= 1) {
                    dataRateMbps = (totalBytesReceived * 8.0 / 1000000.0) / dataRateElapsed;
                    std::cout << "Data Rate: " << dataRateMbps << " Mbps | Last Frame: " << lastProcessedFrame << std::endl;
                    totalBytesReceived = 0;
                    dataRateStartTime = now;
                }
